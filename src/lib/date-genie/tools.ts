...
function planSummary(p: Plan): string {
  ...
  const parking = p.parking
    ...
    : "Parking: none nearby, so take transit or a cab";
...
function planPayload(p: Plan) {
  return {
    ...
    parking: p.parking
      ? { id: p.parking.spot.id, name: p.parking.spot.name, cost: p.parking.cost }
      : null,
    legs: p.legs.map((l) => ({
      kind: l.kind,
      title: l.title,
  ...
  return [
    /* ---------------------------------------------------------- read ---- */
    {
      ...
      execute: async () => {
        ...
        const bits = [
          s.place
            ? `Location: ${s.place.label}. Change it with set_location.`
            : "Location: not set yet. Call set_location before searching.",
          s.pool
          ...
          c.interests.length
            ? `Interested in: ${c.interests.join(", ")}.`
            : "No stated interests yet.",
          s.vetoes.length
            ? `Standing dislikes the human saved: ${s.vetoes.join(", ")}. Never propose these.`
            : "No saved dislikes.",
          c.dietary.length ? `Dietary: ${c.dietary.join(", ")}.` : "",
          s.plan
            ? `A plan is on screen: ${s.plan.dinner.restaurant.name} then ${s.plan.event.event.venue}, ${money(s.plan.total)}.`
            : "No plan on screen yet.",
          s.booking ? `Already booked, confirmation ${s.booking.confirmation}.` : "",
          s.approval.status === "approved"
            ? "The human approved the current plan; you may call book_approved_plan."
            : "",
        ...
        return ok(bits.join("\n"), {
          ...
          lastSearch: s.pool
            ? {
                counts: {
                  restaurants: s.pool.restaurants.length,
                  events: s.pool.events.length,
                  parking: s.pool.parking.length,
                },
...

    {
      ...
      execute: async () => {
        const s = store.getState();
        const live = ACTIVE_ADAPTERS.map(
          (a) => `${a.label} (${a.kind}, live) provides ${a.provides.join(", ")}. ${a.attribution}`,
        );
        const wanted = WANTED_SOURCES.map(
          (a) => `${a.label}: no WebMCP surface. Would need: ${a.wantedContract}`,
        );
        const reports =
          s.pool?.reports.map(
            (r) =>
              `${r.label}: ${r.counts.restaurants} restaurants, ${r.counts.events} venues, ${r.counts.parking} lots in ${r.ms}ms`,
          ) ?? [];
        return ok(
          ...
          {
            active: ACTIVE_ADAPTERS.map(({ id, label, kind, provides, attribution }) => ({
              id,
              label,
              kind,
              provides,
            ...
            wanted: WANTED_SOURCES.map(({ id, label, provides, wantedContract }) => ({
              id,
              label,
              provides,
              wantedContract,
            })),
            lastSearch: s.pool?.reports ?? [],
          },
        );
      },
    },

    {
      ...
      inputSchema: obj(
        { place: str("Any place name, e.g. 'Fredericksburg, VA', 'Shoreditch, London'") },
        ["place"],
      ),
      ...
      execute: async (a) => {
        ...
        const s = store.getState();
        const counts = s.pool
          ? `${s.pool.restaurants.length} restaurants, ${s.pool.events.length} event venues and ${s.pool.parking.length} parking facilities`
          : "no results";
        return ok(
          `Now searching around ${res.place!.label}. Found ${counts}.${s.plan ? `\n\nPlanned:\n${planSummary(s.plan)}` : s.notice ? `\n\n${s.notice}` : "\n\nNo plan yet. Call plan_date_night."}`,
          {
            place: res.place,
            sources: s.pool?.reports ?? [],
            plan: s.plan ? planPayload(s.plan) : null,
          },
        );
      },
    },

    {
      ...
      inputSchema: obj({
        ...
        dietary: {
          ...
          description: "Hard dietary requirements",
        },
      ...
      execute: async (a) => {
        ...
        if (!results.length)
          return ok(
            `Nothing matched near ${where.label}. Widen radiusKm, raise maxPricePerPerson, or drop the cuisine filter.`,
            { restaurants: [] },
          );
        return ok(
          `${results.length} near ${where.label}:\n` +
            list(
              results
                ...
                .map(
                  (r) =>
                    `${r.name} · ${r.cuisine}, ${money(r.pricePerPerson)}/person, ${r.rating}★, ${driveMinutes(r.at)} min drive. Open: ${r.slots.map(fmtTime).join(", ")}. id=${r.id}`,
                ),
            ) +
...

    {
      ...
      inputSchema: obj({
        category: str("Kind of night out", {
          enum: ["comedy", "music", "film", "class", "theater"],
        }),
      ...
      execute: async (a) => {
        ...
        const results = await osmAdapter.searchEvents!({
          ...
          ...(a["maxPricePerTicket"] !== undefined
            ? { maxPricePerTicket: Number(a["maxPricePerTicket"]) }
            : {}),
        });
        if (!results.length)
          return ok(
            `No venues of that kind near ${where.label}. Try a wider radiusKm or drop the category.`,
            { events: [] },
          );
        return ok(
          `${results.length} near ${where.label}:\n` +
            list(
              results
                ...
                .map(
                  (e) =>
                    `${e.name} · ${e.category} at ${e.venue}, ${fmtTime(e.start)}, ${money(e.pricePerTicket)}/ticket, ${e.seatsLeft} seats. id=${e.id}`,
                ),
            ),
...

    {
      name: "find_parking",
      description:
        "Search real parking facilities near the current location, with walk times computed from coordinates.",
      ...
      execute: async (a) => {
        ...
        if (!results.length)
          return ok(
            `No named parking near ${where.label}. Street parking may be the answer there.`,
            { parking: [] },
          );
          ...
          `${withWalk.length} near ${where.label}:\n` +
            list(
              withWalk
                ...
                .map(
                  (p) =>
                    `${p.name} · ${money(p.priceForEvening)} for the evening, ${p.walkMinutes} min walk. id=${p.id}`,
                ),
            ),
...

    {
      ...
      description:
        "Check which table times a restaurant from the last search still has open, and how long a meal there usually takes.",
      inputSchema: obj(
        {
          restaurantId: str("Restaurant id from find_restaurants"),
          earliest: str("24-hour HH:MM"),
          party: num("Party size"),
        },
        ["restaurantId"],
      ),
      ...
      execute: async (a) => {
        ...
        if (!r)
          return fail(
            `No restaurant with id "${a["restaurantId"]}" in the last search. Call find_restaurants first.`,
          );
        ...
        if (!res.available)
          return fail(
            `No tables at ${r.name} after that time. Call find_restaurants for alternatives.`,
          );
        return ok(
          `${r.name} has ${res.slots.length} slots: ${res.slots.map(fmtTime).join(", ")}. Budget about ${res.estimatedMealMinutes} minutes for the meal.`,
          res,
        );
      },
    },
    ...
    /* ---------------------------------------------------- plan / edit ---- */
    {
      ...
      inputSchema: obj(
        {
          request: str(
            "The human's request in their own words, e.g. 'Friday in Fredericksburg VA, under $150, nothing before 7, she hates seafood'",
          ),
        },
        ["request"],
      ),
      ...
      execute: async (a) => {
        ...
        if (!request)
          return fail(
            "Pass the human's request as `request`. Their exact words are better than your summary.",
          );
        ...
        const understood = await understandRequest(request, {
          ...store.getState().constraints,
          avoid: store.getState().vetoes,
        });
        ...
        if (
          named &&
          (!current || !current.label.toLowerCase().startsWith(named.split(",")[0]!.toLowerCase()))
        ) {
          const res = await store.setPlace(named);
          moved = res.ok
            ? `Moved to ${res.place!.label} and searched it fresh.\n\n`
            : `${res.error}\n\n`;
        } else if (!current) {
        ...
        const widened = s.relaxations.length
          ...
          : "";
        const alt = s.alternates.map(
          (p, i) =>
            `Alternate ${i + 1}: ${p.dinner.restaurant.name} + ${p.event.event.venue}, ${money(p.total)}`,
        );
        return ok(
...

    {
      ...
      execute: async (a) => {
        ...
        if (!after) {
          ...
          return fail(
            `"${op}" leaves nothing bookable, so I restored the previous plan. Try a different change, or set_constraint to widen something first.`,
          );
        }
        return ok(
...

    {
      name: "set_constraint",
      description:
        "Set one hard constraint directly and re-plan. Use when the human states a number rather than a feeling.",
      inputSchema: obj({
      ...
      execute: async (a) => {
        ...
        if (a["earliest"]) {
          next.earliest = String(a["earliest"]);
          changed.push(`nothing before ${fmtTime(next.earliest)}`);
        }
        if (a["latestEnd"]) {
          next.latestEnd = String(a["latestEnd"]);
          changed.push(`home by ${fmtTime(next.latestEnd)}`);
        }
...

    {
      ...
      inputSchema: obj(
        { dislike: str("Short term to avoid forever, e.g. 'oysters', 'loud', 'korean'") },
        ["dislike"],
      ),
...

    {
      name: "pick_alternate",
      description:
        "Promote one of the alternates returned by plan_date_night to be the plan on screen.",
      ...
      execute: async (a) => {
        ...
        if (!chosen)
          return fail(
            `No alternate ${a["which"]}. There ${s.alternates.length === 1 ? "is 1" : `are ${s.alternates.length}`}.`,
          );
...

    {
      ...
      execute: async () => {
        ...
        const rows = s.checks.map(
          (c) => `${c.ok ? "PASS" : "FAIL"} · ${c.label}: wanted ${c.target}, got ${c.actual}`,
        );
        return ok(
    ...
    /* -------------------------------------------------- human gateway ---- */
    {
      ...
      execute: async (a, client) => {
        ...
        if (s.booking)
          return fail(`Already booked as ${s.booking.confirmation}. Nothing further to approve.`);
        const summary =
          String(a["note"] ?? "").trim() || `Book this evening for ${money(s.plan.total)}?`;
        ...
        const outcome = client?.requestUserInteraction
          ? await client.requestUserInteraction(askHuman)
          : await askHuman();
...
export function buildConditionalTools(): DateGenieTool[] {
  ...
  if (s.approval.status === "approved" && !s.booking) {
    out.push({
      ...
      inputSchema: obj({ approvalToken: str("The token returned by request_approval") }, [
        "approvalToken",
      ]),
      annotations: {
        title: "Book it",
  ...
  if (s.booking) {
    out.push({
      name: "get_booking",
      description:
        "Retrieve the confirmed evening: confirmation codes, times, and a calendar file. Only exists once something is booked.",
      inputSchema: obj({}),
